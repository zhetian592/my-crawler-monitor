#!/usr/bin/env python3
# crawler.py - 最终稳定版：修复所有已知 Bug（无限循环、限速、缓存、迁移、线程安全等）
import os
import json
import re
import time
    ...
    if "x.com/" in original_url:
        ...
        for cycle in range(max_cycles):
            ...
            while nitter_pool.available:
                ...
                logger.debug(f"尝试 X {username} 使用 {instance} (第{cycle+1}轮)")
                try:
                    items = fetch_single_rss(test_url, original_url, processed_hashes, url_cache, time_window_hours)
                except Exception:
                    items = []
                if items:
                    logger.debug(f"X {username} 成功 via {instance}")
                ...
                time.sleep(0.5)
        logger.debug(f"X {username} 所有实例两轮均失败")
        return []
