...
**负面舆情的定义**（满足其一即可）：
- 批评、攻击、贬低中国、中共、中国政府、中国领导人（习近平等）。
- 涉及台湾、香港、新疆、西藏、南海等议题且带有分裂、对抗、负面评价的内容。
- 涉及中美关系、中欧关系等外交冲突、贸易战、制裁等负面报道。
- 涉及中国境内的敏感事件、群体性事件、抗议、罢工、维权、灾难事故、腐败、环境污染、食品安全、社会不公等。
- 涉及中国企业的负面新闻（如恒大危机、华为被制裁、比亚迪事故等）。
- 任何可能损害中国国家形象、社会稳定、经济发展的内容。

**请严格按照以下 Markdown 表格格式输出**，表格必须包含表头分隔行（`|---|---|---|`）：

| 事件简述 | 原文链接 | 潜在风险点 |
|----------|----------|------------|
| 简述负面事件（不超过80字） | [查看](原文URL) | 风险类型（如外交风险/社会稳定/经济风险）+ 简要说明（不超过30字） |

**要求**：
- 只输出**负面涉华**内容，不要输出正面或中性内容。
- 每一条负面内容单独占一行。
- “原文链接”列必须使用 `[查看](原文URL)` 格式。
- “潜在风险点”需明确指出风险类别（如“外交冲突”“社会维稳”“经济下行”“政治敏感”等）。
- 如果没有任何负面涉华内容，只输出一行“过去24小时无涉华负面内容”。
- 不要添加任何额外解释、开头语或结尾语。

以下是抓取到的全部内容（共 {len(articles_for_ai)} 条，实际抓取 {len(all_articles)} 条）：
